export const ch20 = {
  'ch20-1': {
    ...
    blocks: [
      ...
      {
        type: 'p',
        text: '수동태는 능동태를 문법적으로 바꾼 표현이 아니라, 화자가 어떤 상황·문맥에서 무엇을 강조하고 싶은지에 따라 능동태와 수동태 중 하나를 골라 쓰는 것입니다. 같은 사건이라도 화자가 서 있는 자리가 다르면 문장의 형태도 달라집니다. 아래 네 가지 대표적인 상황을 통해 수동태가 실제로 어떻게 선택되는지 살펴보겠습니다.',
      },
      ...
      { type: 'heading', text: '상황 ④ 일상 대화 — 「나(주어)에게 일어난 일」을 강조할 때' },
      {
        type: 'p',
        text: '수동태는 격식 있는 글에만 쓰이는 것이 아닙니다. 일상 대화에서도 화자 자신이 어떤 일을 겪었다는 느낌을 전달하고 싶을 때 수동태가 자연스럽게 나옵니다. 이때는 「내가 무엇을 했다」가 아니라 「나에게 이런 일이 벌어졌다」는 피해자·경험자의 시점이 핵심입니다.',
      },
      {
        type: 'example',
        items: [
          { en: 'My phone was stolen on the subway this morning!', ko: '오늘 아침 지하철에서 내 휴대폰을 도둑맞았어!', note: '도둑이 누구인지보다 「내가 당했다」는 사실과 감정이 중요' },
          { en: 'I was invited to the party, but I couldn’t go.', ko: '나는 그 파티에 초대받았지만 갈 수 없었다.', note: '누가 초대했는지는 문맥상 이미 알거나 중요하지 않음 — 화자의 상황이 핵심' },
          { en: 'The meeting was postponed to next week.', ko: '회의가 다음 주로 연기되었다.', note: '누가 연기를 결정했는지보다 「일정이 바뀌었다」는 결과가 청자에게 중요한 정보' },
        ],
      },
      {
        type: 'callout',
        title: '수동태를 쓰는 네 가지 이유',
        items: [
          ...
          '④ 화자 자신이 어떤 일을 「당했다·겪었다」는 경험을 강조할 때 — My phone was stolen!',
        ],
      },
      {
        type: 'icon-row',
        ...
        items: [
          { icon: '❓', label: '행위자를 모름', sub: 'A window was broken.' },
          { icon: '📦', label: '대상이 더 중요', sub: 'Three people were injured.' },
          { icon: '🙈', label: '행위자를 숨김', sub: 'My phone was stolen!' },
        ],
      },
  ...
  'ch20-2': {
    ...
    blocks: [
      ...
      },
      {
        type: 'example',
        items: [
          { en: 'Wonjun was defeated by the transfer student!', ko: '원준이가 전학생에 의해 패배하였다!', note: '누구에게 패배했는지가 핵심 정보이므로 by를 반드시 남김' },
        ],
      },
      ...
      {
        type: 'p',
        text: '4형식(S+V+O1+O2)은 목적어가 두 개(사람 목적어, 사물 목적어)이므로 수동태로 바꿀 때 둘 중 어느 것을 주어로 삼을지 선택할 수 있습니다. 일반적으로 사람 목적어를 주어로 세우는 것이 더 자연스럽고, 사물 목적어를 주어로 세우면 남은 사람 목적어 앞에 전치사(to, for 등)를 붙여야 합니다.',
      },
      {
        type: 'sentence-diagram',
        label: '4형식 능동태',
      ...
      },
      {
        ...
        items: [
          ...
          '수동태는 행위자보다 행위의 대상·결과를 강조하거나, 행위자를 모르거나 밝히고 싶지 않을 때, 공식적·객관적 어조가 필요할 때, 화자 자신이 겪은 일을 강조할 때 쓰인다.',
