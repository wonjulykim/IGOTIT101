export const ch25 = {
  'ch25-1': {
    ...
    blocks: [
      ...
      },
      {
        ...
        headers: ['구분', '형태', '의미', '예'],
        rows: [
          ['현재분사', '동사원형 + ing', '능동(스스로 하는), 진행(~하고 있는)', 'an exciting movie (흥미를 일으키는 영화)'],
          ['과거분사', '동사원형 + ed (불규칙 있음)', '수동(~되는, ~당하는), 완료(이미 ~된)', 'an excited audience (흥미를 느끼게 된 관객)'],
        ],
      },
      {
        type: 'icon-row',
      ...
      {
        type: 'note',
        text: '관계절(who is, which is 등)에서 "관계대명사 + be동사"를 지우면 남는 것이 현재분사(-ing)나 과거분사(p.p.)입니다. 이렇게 관계절을 분사로 줄이면 문장이 훨씨 간결해집니다.',
      },
  ...
  'ch25-2': {
    ...
    blocks: [
      ...
      {
        type: 'p',
        text: '다만 이 압축에는 대가가 있습니다. 생략된 접속사와 주어를 듣는 사람(읽는 사람)이 앞뒤 문맥으로 유추해야 하기 때문입니다. 그래서 즉흥적으로 대화하는 구어체에서는 분사구문이 오히려 이해를 어렵게 만들 수 있어 잘 쓰이지 않고, 앞뒤 맥락을 충분히 살펴볼 수 있는 문어체(글쓰기)에서 효과적으로 사용됩니다.',
      },
      {
        type: 'example',
        items: [
      ...
      { type: 'heading', text: '주어가 다를 때: 의미상 주어를 남기는 분사구문' },
      {
        type: 'p',
        text: '분사구문을 만들 때 주어를 지우는 것은 "부사절의 주어와 주절의 주어가 같을 때"에 한정됩니다. 두 주어가 다르면 부사절의 주어를 지우지 않고 분사 앞에 그대로 남겨 둡니다. 이렇게 남겨진 주어를 분사구문의 "의미상 주어"라고 부릅니다.',
      },
      {
        type: 'example',
        items: [
          { en: 'Weather permitting, we will go on a picnic.', ko: '날씨가 허락한다면, 우리는 소풍을 갈 것이다.', note: '부사절의 주어 Weather가 주절의 주어 we와 다르므로 Weather를 지우지 않고 남김' },
          { en: 'The sun having set, the temperature dropped quickly.', ko: '해가 진 후, 온도가 빠르게 떨어졌다.', note: '부사절의 주어 the sun이 주절의 주어 the temperature와 다르므로 남김' },
          { en: 'All things considered, the trip was a success.', ko: '모든 것을 고려해 볼 때, 그 여행은 성공적이었다.', note: '의미상 주어 All things가 주절의 the trip과 다르므로 남김. considered는 과거분사(고려를 당하는 대상)' },
          { en: 'The bus having already left, we had to wait for the next one.', ko: '버스가 이미 떠난 후여서, 우리는 다음 버스를 기다려야 했다.', note: '부사절의 주어 the bus가 주절의 주어 we와 다르므로 남김' },
        ],
      },
      {
        type: 'note',
        text: '흔한 실수: 부사절과 주절의 주어가 다른데도 부사절 주어를 지워버리면, 분사구문의 주어가 주절의 주어로 잘못 해석되는 "붕 뜬 분사(dangling participle)"가 됩니다. "*Walking down the street, the trees were beautiful."이라고 쓰면 나무가 걸어다닌다는 뜻이 되어버립니다. 실제로 걷고 있는 것이 "나"라면 주어를 살려 "Walking down the street, I thought the trees were beautiful." 또는 문장을 아예 다시 "As I walked down the street, the trees looked beautiful."로 풀어 써야 합니다. 마찬가지로 "*Having read the report, the conclusion seemed obvious."도 보고서를 읽은 것은 결론이 아니라 사람이므로, "Having read the report, I found the conclusion obvious."처럼 실제 행위자를 주어로 살려야 합니다.',
      },
      ...
      },
      {
        ...
        items: [
          ...
          '부사절과 주절의 주어가 다르면 주어를 지우지 않고 분사 앞에 남긴다(Weather permitting, ~). 다른데도 지우면 "붕 뜬 분사" 오류가 생긴다.',
          '분사구문은 문장을 간결하게 하는 장점이 있지만, 생략된 내용을 문맥으로 유추해야 하는 단점도 있어 즉흥적인 구어체보다 문학적 서술, 학술 논문, 뉴스 기사, 보도문, 기술 문서 같은 문어체에서 더 효과적으로 쓰인다.',
        ],
      },
    ],
  },
