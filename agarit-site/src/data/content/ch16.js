export const ch16 = {
  'ch16-1': {
    ...
    blocks: [
      ...
      {
        type: 'p',
        text: '수여동사는 목적어를 2개 가집니다. ‘누군가에게 무엇을 준다’는 뜻이니 당연히 ‘물건을 줄 사람’과 ‘줄 물건’ 두 가지 정보가 필요합니다. 이 두 목적어를 각각 간접목적어(사람)와 직접목적어(사물)라고 부르기도 하지만, 이름 자체보다는 ‘사람이 먼저, 사물이 나중’이라는 순서가 더 중요합니다.',
      },
      {
      ...
      {
        type: 'p',
        text: '첫 번째 문장에는 ‘생일 카드를 보냈다’는 정보만 있고 누구에게 보냈는지는 나타나 있지 않습니다. send, give, show 같은 동사들은 애초에 그 행위 자체가 ‘누군가를 향한’ 행위이기 때문에, 받는 사람이 빠지면 문장의 의미가 불완전하게 느껴집니다. 수여동사를 4형식(S+V+O1+O2)으로 쓰면 받는 사람에 대한 정보까지 한 문장에 압축해서 담을 수 있습니다.',
      },
      {
        type: 'example',
        items: [
      ...
      { type: 'heading', text: '동사의 의미에 따라 달라지는 전치사 — to, for, of' },
      {
        type: 'p',
        text: '4형식을 3형식으로 바꿀 때 모든 동사가 같은 전치사를 쓰는 것은 아닙니다. 전치사는 단순한 문법 규칙이 아니라, 동사가 나타내는 행위의 성격을 그대로 반영합니다. ‘물건을 어떻게 그 사람에게 이르게 하는가’에 따라 to, for, of 중 하나가 정해집니다.',
      },
      {
        type: 'table',
        ...
        rows: [
          ['to', '이미 존재하는 물건이 사람에게 그대로 옮겨감(전달·이동)', 'give, bring, send, teach, lend, tell, show, offer, pass'],
          ['for', '받는 사람을 위해 없던 것을 만들거나 마련함(정성·노력·시간·돈이 투입됨)', 'make, cook, buy, find, get, build, bake'],
          ['of', '상대방에게서 무언가를 얻어내려 요구함(요청·질문의 방향이 반대)', 'ask, require, demand'],
        ],
      },
      {
        type: 'icon-row',
        ...
        items: [
          ...
          { icon: '🙏', label: 'of', sub: '요구함 · ask' },
        ],
      },
      {
        type: 'p',
        text: '왜 이렇게 세 가지로 나뉠까요? give, tell, show류는 이미 존재하는 것(물건, 정보, 모습)을 상대방 쪽으로 ‘이동’시키는 동사입니다. 이동에는 방향이 있으므로 방향을 나타내는 전치사 to가 자연스럽습니다. 반면 make, cook, buy류는 물건이 원래부터 있던 것이 아니라 ‘그 사람을 위해서’ 새로 만들어내거나 마련하는 행위입니다. 여기서 핵심은 이동이 아니라 ‘누구를 위한 것인가’이므로, 목적·수혜를 나타내는 전치사 for가 쓰입니다. ask는 방향이 반대입니다. 무언가를 주는 것이 아니라 상대방으로부터 대답이나 정보를 얻어내려는 행위이므로, ‘~로부터 나온 것’을 나타내는 of가 쓰입니다.',
      },
      {
        type: 'example',
        items: [
          ...
          { en: 'context: 면접에서 면접관이 지원자에게 질문하는 상황', note: '문맥 — ask (요구·역방향)' },
          { en: 'She asked him a question.', ko: '그녀는 그에게 질문을 했다.' },
          { en: 'She asked a question of him.', ko: '그녀는 질문을 그에게 물었다.', note: '대답을 그로부터 얻어내려는 행위 → of (실제로는 of보다 him을 그대로 유지하는 경우가 더 흔함)' },
        ],
      },
      {
        type: 'note',
        text: 'to는 물건이 단순히 사람에게 옮겨가는 것만을 나타내지만, for는 그 물건을 준비하는 데 들어간 정성·노력·시간·돈까지 함께 나타냅니다. 그래서 make, cook, buy처럼 무언가를 만들거나 마련해서 주는 동사들은 for를 사용합니다. 반면 ask는 정보의 흐름이 반대(상대방 → 나)이기 때문에 to/for가 아니라 of가 쓰인다는 점에서 나머지 동사들과 방향 자체가 다릅니다.',
      },
      { type: 'heading', text: '흔히 하는 실수 — 직접목적어가 대명사일 때' },
      {
        type: 'p',
        text: '직접목적어(사물)가 it, them처럼 이미 알려진 대명사일 경우, 4형식(S+V+사람+대명사) 순서로 쓰면 매우 부자연스럽거나 틀린 문장이 됩니다. 대명사는 이미 알고 있는 짧고 가벼운 정보이므로, 문장의 무게 중심을 맨 끝에 두는 영어의 습성상 오히려 동사 바로 뒤에 붙어야 하고, 사람은 전치사와 함께 뒤로 이동해야 자연스럽습니다.',
      },
      {
        type: 'example',
        items: [
      ...
      {
        type: 'note',
        text: '정리하면, 직접목적어가 명사(a book, a present 등)일 때는 "S+V+사람+사물" 4형식과 "S+V+사물+전치사+사람" 3형식이 모두 자연스럽지만, 직접목적어가 대명사(it, them)일 때는 3형식(S+V+it/them+전치사+사람)만 쓰는 것이 원칙입니다. 대명사처럼 가볍고 이미 알려진 정보는 문장 뒤쪽 무거운 자리로 보내지 않는다는 영어의 일반적인 정보 배치 습성과 같은 원리입니다.',
      },
      {
        type: 'callout',
        title: '16장 핵심 정리',
        items: [
          '수여동사(give, send, bring, make, buy, show, tell, ask 등)는 ‘누군가에게 무엇을 준다(또는 요구한다)’는 뜻으로, 목적어를 2개(간접목적어=사람, 직접목적어=사물) 갖는 4형식 문장을 만든다.',
          ...
          '전치사는 동사가 나타내는 행위의 성격을 반영한다: give·bring·send·teach·lend·tell·show(있는 것을 그대로 전달)는 to, make·cook·buy(없던 것을 마련해 줌)는 for, ask(상대에게서 얻어냄, 방향이 반대)는 of.',
